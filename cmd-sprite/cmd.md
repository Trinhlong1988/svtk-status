# 🎨 CMD_SPRITE v1.0 — SPRITE GENERATOR via LoRA

> **PASTE NGUYÊN VÀO CLAUDE CODE.** Autonomous.

**Version:** 1.0.0 — 2026-05-18
**Team:** TEAM ART — Sprite generation via LoRA + 158 templates × recolor
...
- **R45** — Anti-dupe sprite_id unique
- **R47** — Cross-reference NPC template_id từ cmd-npc
- **R49** — Content tagging style + era + faction
- **R50** — Schema-strict sprite_id 1..158 templates
- **R68** — Deterministic via seed (KHÔNG random style)
...
```yaml
goal: "158 sprite templates × 8 hướng × 4 frame + recolor mapping cho 7817 NPC + PNG indexed 16 màu ~30KB"

target_template_count: 158
target_directions: 8
target_frames: 4
target_recolor_mapping: 7817
```
...
cmd-sprite/output/
├── registry/
│   ├── sprite_template.jsonl
│   ├── recolor_mapping.jsonl
│   ├── lora_config.json
│   ├── sprite_table.sql
└── tests/sprite_tests.py (≥15 tests)
```
...
```python
#!/usr/bin/env python3
"""CMD_SPRITE v1.0 — autonomous builder."""
...
TARGET_DIRECTIONS = 8
TARGET_FRAMES = 4
TARGET_RECOLOR_COUNT = 7817
SPRITE_SIZE_PX = 64
SPRITE_COLOR_DEPTH = 16  # PNG indexed
TARGET_FILE_SIZE_KB = 30
...
def run_full_build():
    ...
    # Sprite template metadata (LoRA generation đặt riêng — đây ship config)
    templates = []
    for tid in range(1, TARGET_TEMPLATE_COUNT + 1):
        templates.append({
            'template_id': tid,
            'style': 'pixel_art_viet',
            'directions': TARGET_DIRECTIONS,
            'frames': TARGET_FRAMES,
            'color_depth': SPRITE_COLOR_DEPTH,
            'target_size_kb': TARGET_FILE_SIZE_KB,
        })

    # Recolor mapping: NPC unique → template + color palette
    recolor_map = []
    for npc_idx in range(1, TARGET_RECOLOR_COUNT + 1):
        recolor_map.append({
            'npc_index': npc_idx,
            'template_id': (npc_idx % TARGET_TEMPLATE_COUNT) + 1,
            'palette_seed': npc_idx,
        })
    ...
    schema_sql = """CREATE TABLE IF NOT EXISTS sprite_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
CREATE INDEX idx_sprite_key ON sprite_items(natural_key);
"""
    (OUTPUT_DIR / 'schema' / f'sprite_table.sql').write_text(schema_sql, encoding='utf-8')

    # Test stub (≥15 tests required)
    test_code = '''# 15 tests for SPRITE
import json
from pathlib import Path
...
'''
    (OUTPUT_DIR / 'tests' / f'sprite_tests.py').write_text(test_code, encoding='utf-8')
...
```sql
CREATE TABLE IF NOT EXISTS sprite_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
...
suite = RNGSuite(seed=f'sprite:{entity_id}')
```
...
1-15: implementation specific to CMD_SPRITE (count, schema, cross-ref, idempotency)

### ⚠️ Gap nội tại (4 admit honest)

1. **Sprite actual PNG file gen yêu cầu LoRA + Stable Diffusion local (offline gen)** — MED
2. **Recolor mapping chưa verify với asset thực** — MED
3. **No animation frame interpolation (4 frame discrete)** — MED
4. **8 hướng có thể giảm thành 4 (north/east/south/west) cho mobile** — MED
...
Repo `Trinhlong1988/svtk-status` branch `staging-sprite-{ts}`.
