# 🔊 CMD_AUDIO v1.0 — AUDIO GENERATOR BGM+SFX

> **PASTE NGUYÊN VÀO CLAUDE CODE.** Autonomous.

**Version:** 1.0.0 — 2026-05-18
**Team:** TEAM ART — Audio OGG mono BGM/SFX
...
- **R45** — Anti-dupe audio_id unique
- **R47** — Cross-reference scene/event triggers
- **R49** — Content tagging type + era + mood
- **R50** — Schema-strict audio_id 1..500
- **R68** — Deterministic gen
...
```yaml
goal: "500 audio assets (200 BGM mono 64kbps ~500KB + 300 SFX mono 32kbps ~30KB) + 5 era variations"

target_audio_count: 500
target_bgm_count: 200
target_sfx_count: 300
```
...
```python
#!/usr/bin/env python3
"""CMD_AUDIO v1.0 — autonomous builder."""
...
MOODS = ['peaceful', 'tense', 'battle', 'sad', 'victorious']
...
def run_full_build():
    ...
    audios = []
    for aid in range(1, TARGET_AUDIO_COUNT + 1):
        audio_type = 'bgm' if aid <= TARGET_BGM_COUNT else 'sfx'
        bitrate = BGM_BITRATE_KBPS if audio_type == 'bgm' else SFX_BITRATE_KBPS
        audios.append({
            'audio_id': aid,
            'filename': f'audio_{aid:04d}.ogg',
            'type': audio_type,
            'bitrate_kbps': bitrate,
            'era': ERAS[aid % len(ERAS)],
            'mood': MOODS[aid % len(MOODS)],
        })
    ...
    schema_sql = """CREATE TABLE IF NOT EXISTS audio_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
CREATE INDEX idx_audio_key ON audio_items(natural_key);
"""
    (OUTPUT_DIR / 'schema' / f'audio_table.sql').write_text(schema_sql, encoding='utf-8')

    # Test stub (≥15 tests required)
    test_code = '''# 15 tests for AUDIO
import json
from pathlib import Path
...
'''
    (OUTPUT_DIR / 'tests' / f'audio_tests.py').write_text(test_code, encoding='utf-8')
...
suite = RNGSuite(seed=f'audio:{entity_id}')
```
...
1-15: implementation specific to CMD_AUDIO (count, schema, cross-ref, idempotency)

### ⚠️ Gap nội tại (4 admit honest)

1. **Audio actual OGG file gen offline (AI music model local)** — MED
2. **Mono only (no stereo, no spatial)** — MED
3. **No dynamic mixing/crossfade config** — MED
4. **Music adaptive transitions chưa có** — MED
...
Repo `Trinhlong1988/svtk-status` branch `staging-audio-{ts}`.
