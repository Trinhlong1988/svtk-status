#!/usr/bin/env python3
"""CMD QUEST v1.9 — 10-round audit batch 8 fix (V76-V84).

Fix bug v1.8 audit:
- V77: CRLF line ending in JSONL. Node.js split('\\n') leaves \\r at end →
       JSON parse fail. Fix: write with newline='\\n' explicit.

V76/V78/V79/V80/V81/V82/V83/V84 verified PASS:
- V76 reputation 0-364 non-negative.
- V78 0 legacy main without chain.
- V79 SQL CHECK era enum 11 values match.
- V80 _source debug + created_at DB auto-fill OK.
- V81 21/21 reborn desc có era diacritic.
- V82 38 chain playability fails = design intent (era progression chains).
- V83 max prereq 1, condition tokens 4 < 100.
- V84 0 quests in multiple chains.

Previous fixes v1.2-v1.8: 18+ bug fix qua 75 vòng audit.
"""
import os
import sys
import time
...
def _write_lead(sub: str, fname: str, payload: dict):
    ...
    # V77: write_bytes để LF (no CRLF on Windows) — JSON parse compat
    (d / fname).write_bytes(
...
def write_outputs(quests, chains):
    ...
    for cat, lst in by_category.items():
        ...
        # V77: explicit newline='\n' để LF (no CRLF on Windows) — Node.js split('\n') compat
        with path.open('w', encoding='utf-8', newline='\n') as f:
            for q in lst:
                f.write(json.dumps(q, ensure_ascii=False) + '\n')
    with (OUTPUT_DIR / 'registry' / 'quest_full.jsonl').open(
            'w', encoding='utf-8', newline='\n') as f:
        for q in quests:
            f.write(json.dumps(q, ensure_ascii=False) + '\n')
    (OUTPUT_DIR / 'chains' / 'quest_chains.json').write_bytes(
...
'''
    (OUTPUT_DIR / 'schema' / 'quest_table.sql').write_bytes(sql.encode('utf-8'))
...
def validator(quests, chains):
    ...
    (OUTPUT_DIR / 'reports' / 'validation.json').write_bytes(
...
def chain_integrity_report(quests, chains):
    ...
    (OUTPUT_DIR / 'reports' / 'chain_integrity.json').write_bytes(
        (json.dumps(report, indent=2, ensure_ascii=False) + '\n').encode('utf-8'))
    return report
...
def self_tests(quests, chains, npcs):
    ...
    (OUTPUT_DIR / 'reports' / 'self_tests.json').write_bytes(
...
def write_honest_gaps(legacy_count):
    ...
    (OUTPUT_DIR / 'reports' / 'honest_gaps.json').write_bytes(
...
def main():
    try:
        ...
        (OUTPUT_DIR / 'reports' / 'final_summary.json').write_bytes(
            (json.dumps(summary, indent=2, ensure_ascii=False) + '\n').encode('utf-8'))
