#!/usr/bin/env python3
"""CMD QUEST v1.5 — 10-round deepest audit fixes on top of v1.4.

Fix bug v1.4 audit:
- V36: Push alert_to_lead cho legacy era mismatch giver (R71 immutable, không reassign).
- V38: Clear chain_id/chain_position cho quest không thuộc new chain (side legacy orphan).
- V39: Lore chain chunk size 5 (was 4), bundle era remainder vào chunk trước
       để mọi chain >= 5 quests (TS test rule).

Previous fixes:
- v1.4 (V26-V34): era pool filter, title unique, reward cap, era-level band,
  lore codex, level-reward correlation, description variety.
- v1.3 (V11-V21): chain length cap 15, dialog preserve, level cap 120,
  smart NPC, NPC#1 exclude, per-era suffix, last-era unlocks.
- v1.2 (R66-R83): Foundation v2.8.0 compliance.
"""
import os
import sys
import time
...
def build_chains(quests):
    ...
    def _split_into_chunks(items, max_len, min_len):
        """Split items into chunks of max_len, with last chunk merged if < min_len."""
        if not items:
            return []
        chunks = []
        for start in range(0, len(items), max_len):
            chunks.append(items[start:start + max_len])
        # If last chunk too short and there's a prev chunk, merge them
        if len(chunks) >= 2 and len(chunks[-1]) < min_len:
            tail = chunks.pop()
            chunks[-1] = chunks[-1] + tail
            # Re-split if merged chunk > max_len (rare with min<max)
            if len(chunks[-1]) > max_len:
                merged = chunks.pop()
                # Split into halves to balance
                half = len(merged) // 2
