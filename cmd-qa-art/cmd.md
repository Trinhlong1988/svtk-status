# 🎨 CMD_QA_ART v1.0 — QA ART VERIFICATION

> **PASTE NGUYÊN VÀO CLAUDE CODE.** Autonomous.

**Version:** 1.0.0 — 2026-05-18
**Team:** TEAM QA — Verify Sprite/Map/Icon/Audio output
...
- **R47** — Cross-reference art assets
- **R49** — Content tagging art consistency
- **R50** — File size + format strictness
- **R68** — Deterministic art gen verify
- **R30** — Cultural style verification (no Japanese/Chinese style)
...
```yaml
goal: "Verify SPRITE 158 templates × 8 hướng × 4 frame + MAP 7047 images + ICON 2000 + AUDIO 500 + file size + format compliance"

target_sprite_check: 158
target_map_check: 7047
target_icon_check: 2000
target_audio_check: 500
```
...
├── registry/
│   ├── qa_art_report.json
│   ├── qa_failed_assets.jsonl
│   ├── qa_art_table.sql
└── tests/qa_art_tests.py (≥15 tests)
```
...
```python
#!/usr/bin/env python3
"""CMD_QA_ART v1.0 — autonomous builder."""
...
TARGET_ICON_CHECK = 2000
TARGET_AUDIO_CHECK = 500
SPRITE_FILE_SIZE_KB_MAX = 35
MAP_FILE_SIZE_KB_MAX = 55
ICON_FILE_SIZE_KB_MAX = 6
BGM_FILE_SIZE_KB_MAX = 550
CLIP_SCORE_MIN = 0.85
...
def run_full_build():
    ...
    qa_art = {}
    for cmd_name in ['sprite', 'map', 'icon', 'audio']:
        manifest = REPO_DIR / f'cmd-{cmd_name}' / 'output' / 'registry' / f'{cmd_name}_manifest.jsonl'
        if not manifest.exists():
            send_alert_to_lead('HIGH', f'qa_art_{cmd_name}_missing',
                              {'expected': str(manifest)})
            qa_art[cmd_name] = 'MISSING'
        else:
            qa_art[cmd_name] = 'VERIFIED'
    ...
    schema_sql = """CREATE TABLE IF NOT EXISTS qa_art_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
CREATE INDEX idx_qa_art_key ON qa_art_items(natural_key);
"""
    (OUTPUT_DIR / 'schema' / f'qa_art_table.sql').write_text(schema_sql, encoding='utf-8')

    # Test stub (≥15 tests required)
    test_code = '''# 15 tests for QA_ART
import json
from pathlib import Path
...
'''
    (OUTPUT_DIR / 'tests' / f'qa_art_tests.py').write_text(test_code, encoding='utf-8')
...
```sql
CREATE TABLE IF NOT EXISTS qa_art_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
...
suite = RNGSuite(seed=f'qa_art:{entity_id}')
```
...
1-15: implementation specific to CMD_QA_ART (count, schema, cross-ref, idempotency)

### ⚠️ Gap nội tại (4 admit honest)

1. **CLIP score verify yêu cầu local model (manifest-based audit ship trước)** — MED
2. **No pixel-level diff check (file size + format only)** — MED
3. **Audio waveform quality không check** — MED
4. **Cross-platform render differences chưa cover** — MED
...
Repo `Trinhlong1988/svtk-status` branch `staging-qa_art-{ts}`.
...
## 🔍 VERIFY FUNCTIONS — QA-ART

```python
def verify_art(entries: list, max_file_size_kb: int = None,
               expected_format: str = None) -> list:
    """Verify art asset: file size + format + dimensions."""
    issues = []
    for i, e in enumerate(entries):
        ...
        # 2. Format check
        if expected_format and e.get('format', '') != expected_format:
            issues.append({'type': 'format_mismatch',
                          'index': i,
                          'expected': expected_format,
                          'actual': e.get('format')})
        # 3. Color depth (asset size lock R20)
        if e.get('format') == 'PNG' and e.get('color_depth', 32) > 16:
            issues.append({'type': 'png_not_indexed_16',
                          'index': i,
                          'actual_depth': e.get('color_depth')})
    return issues


def push_verdict_to_lead(target_worker: str, verdict: str, evidence: dict):
    """Push verdict lên cmd-lead/qa-verdicts/."""
...
QA_TARGETS = ['sprite', 'map', 'icon', 'audio']
SIZE_LIMITS = {
    'sprite': 30,  # PNG indexed 16 màu ~30KB
    'map': 55,     # JPG Q70 832x640 ~50KB
    'icon': 6,     # PNG 32x32 indexed ~5KB
    'audio': 550,  # OGG mono ~500KB BGM
}
```
...
def qa_main_workflow():
    """QA-ART main: verify 4 art targets."""
    for worker in QA_TARGETS:
        manifest = REPO_DIR / f'cmd-{worker}' / 'output' / 'registry' / f'{worker}_manifest.jsonl'
        if not manifest.exists():
            continue
        entries = []
        ...
        issues = verify_art(entries, max_file_size_kb=SIZE_LIMITS.get(worker))
        if not issues:
            push_verdict_to_lead(worker, 'PASS', {'count': len(entries)})
        else:
            for issue in issues:
                severity = 'HIGH' if issue['type'] == 'png_not_indexed_16' else 'MED'
                send_alert_to_lead_with_target(severity,
                                              f'{worker}_{issue["type"]}',
