# 🎯 CMD_ICON v1.0 — ICON GENERATOR 32×32

> **PASTE NGUYÊN VÀO CLAUDE CODE.** Autonomous.

**Version:** 1.0.0 — 2026-05-18
**Team:** TEAM ART — Icon PNG 32×32 indexed ~5KB
...
- **R45** — Anti-dupe icon_id unique
- **R47** — Cross-reference Item/Skill/Quest categories
- **R49** — Content tagging category + rarity
- **R50** — Schema-strict icon_id 1..2000
...
```yaml
goal: "2000 icons PNG 32×32 indexed 16 màu ~5KB + categories (item/skill/quest/buff/debuff/ui)"

target_icon_count: 2000
target_resolution: 32
target_color_depth: 16
```
...
├── registry/
│   ├── icon_manifest.jsonl
│   ├── icon_category_config.json
│   ├── icon_table.sql
└── tests/icon_tests.py (≥15 tests)
```
...
```python
#!/usr/bin/env python3
"""CMD_ICON v1.0 — autonomous builder."""
...
TARGET_RESOLUTION = 32
TARGET_COLOR_DEPTH = 16
TARGET_FILE_SIZE_KB = 5
CATEGORIES = ['item', 'skill', 'quest', 'buff', 'debuff', 'ui']
...
def run_full_build():
    ...
    icons = []
    for iid in range(1, TARGET_ICON_COUNT + 1):
        icons.append({
            'icon_id': iid,
            'filename': f'icon_{iid:05d}.png',
            'category': CATEGORIES[iid % len(CATEGORIES)],
            'resolution': TARGET_RESOLUTION,
            'color_depth': TARGET_COLOR_DEPTH,
        })
    ...
    schema_sql = """CREATE TABLE IF NOT EXISTS icon_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
CREATE INDEX idx_icon_key ON icon_items(natural_key);
"""
    (OUTPUT_DIR / 'schema' / f'icon_table.sql').write_text(schema_sql, encoding='utf-8')

    # Test stub (≥15 tests required)
    test_code = '''# 15 tests for ICON
import json
from pathlib import Path
...
'''
    (OUTPUT_DIR / 'tests' / f'icon_tests.py').write_text(test_code, encoding='utf-8')
...
suite = RNGSuite(seed=f'icon:{entity_id}')
```
...
1-15: implementation specific to CMD_ICON (count, schema, cross-ref, idempotency)

### ⚠️ Gap nội tại (4 admit honest)

1. **Icon actual PNG gen yêu cầu LoRA local** — MED
2. **No icon variations per rarity (chỉ 1 icon per category)** — MED
3. **No animated icons** — MED
4. **Theme variants (light/dark mode) chưa hỗ trợ** — MED
...
Repo `Trinhlong1988/svtk-status` branch `staging-icon-{ts}`.
