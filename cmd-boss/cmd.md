# 👹 CMD_BOSS v1.0 — BOSS GENERATOR >=1200

> **PASTE NGUYÊN VÀO CLAUDE CODE.** Autonomous.

**Version:** 1.0.0 — 2026-05-18
**Team:** TEAM CONTENT — Boss + raid mechanics
...
- **R45** — Anti-dupe boss_id unique
- **R47** — Cross-reference với MAP (boss spawn location)
- **R49** — Content tagging tier + era + faction
- **R50** — Schema-strict boss_id 1..1200
- **R68** — Deterministic spawn pattern
...
```yaml
goal: ">=1200 boss với tier (normal/elite/raid/world) + 5 era + spawn config + drop table"

target_boss_count: 1200
target_tier_normal: 600
target_tier_elite: 300
target_tier_raid: 200
target_tier_world: 100
```
...
```python
#!/usr/bin/env python3
"""CMD_BOSS v1.0 — autonomous builder."""
...
TARGET_TIER_RAID = 200
TARGET_TIER_WORLD = 100
ELEMENTS = ['KIM', 'MOC', 'THUY', 'HOA', 'THO', 'TAM', 'BACH', 'HAC']
ERAS = ['ly', 'tran', 'le', 'tay_son', 'nguyen']
TIERS = ['normal', 'elite', 'raid', 'world']
...
def run_full_build():
    ...
    bosses = []
    bid = 1
    for tier, count in [('normal', TARGET_TIER_NORMAL), ('elite', TARGET_TIER_ELITE),
                        ('raid', TARGET_TIER_RAID), ('world', TARGET_TIER_WORLD)]:
        for _ in range(count):
            bosses.append({'boss_id': bid, 'tier': tier, 'era': ERAS[bid % len(ERAS)],
                          'element': ELEMENTS[bid % len(ELEMENTS)]})
            bid += 1
    ...
    schema_sql = """CREATE TABLE IF NOT EXISTS boss_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
CREATE INDEX idx_boss_key ON boss_items(natural_key);
"""
    (OUTPUT_DIR / 'schema' / f'boss_table.sql').write_text(schema_sql, encoding='utf-8')

    # Test stub (≥15 tests required)
    test_code = '''# 15 tests for BOSS
import json
from pathlib import Path
...
'''
    (OUTPUT_DIR / 'tests' / f'boss_tests.py').write_text(test_code, encoding='utf-8')
...
**Existing:** 13 entries đã có từ ChatGPT session trước.
**Target:** ≥1200
**Extend:** 1187 entries mới (existing IMMUTABLE).

```python
def r71_workflow():
    """R71: Tận dụng existing, mở rộng không làm mới."""
    existing_path = REPO_DIR / 'cmd-boss' / 'existing' / f'BOSS_13.jsonl'
    ...
    if existing_path.exists():
        for line in existing_path.read_text(encoding='utf-8').split('\n'):
            if line.strip():
                existing.append(json.loads(line))
        log.info(f'Loaded {len(existing)} existing BOSS from {existing_path}')
    else:
        log.warn(f'Existing registry NOT FOUND at {existing_path} — will generate full 1200')
    ...
    # 3. Check target met
    if len(valid_existing) >= 1200:
        log.info(f'Target 1200 met with existing valid {len(valid_existing)}')
        return valid_existing, 0  # 0 new

    # 4. Extend chỉ phần thiếu
    needed = 1200 - len(valid_existing)
    start_id = max(
...
suite = RNGSuite(seed=f'boss:{entity_id}')
```
...
1-15: implementation specific to CMD_BOSS (count, schema, cross-ref, idempotency)

### ⚠️ Gap nội tại (4 admit honest)

1. **Boss AI hành vi đơn giản (state machine)** — MED
2. **World boss spawn lịch tĩnh** — MED
3. **Drop table simplified (cố định rate)** — MED
4. **No cross-server boss raid** — MED
...
Repo `Trinhlong1988/svtk-status` branch `staging-boss-{ts}`.
...
```python
SVTK_TARGET = 1200    # VSTK target (vượt TSO)
TSO_BASELINE = 922    # TS Online actual
# Phải PASS: count >= SVTK_TARGET (> TSO 922)
```
...
EXISTING_PATH = REPO_DIR / 'cmd-boss' / 'existing'
OUTPUT_PATH = REPO_DIR / 'cmd-boss' / 'output' / 'registry'
...
def main_pipeline():
    ...
    if bugs:
        for bug in bugs:
            ...
            send_alert_to_lead_with_target(severity, f'boss_' + bug['type'],
                                          bug['evidence'], target_worker='boss')
    ...
    out = OUTPUT_PATH / 'boss_full.jsonl'
    ...
    push_completion_to_lead(
        fix_id=f'boss_extend_to_target',
