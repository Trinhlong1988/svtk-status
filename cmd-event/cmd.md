# 🎉 CMD_EVENT v1.0 — EVENT SCRIPT GENERATOR >=600

> **PASTE NGUYÊN VÀO CLAUDE CODE.** Autonomous.

**Version:** 1.0.0 — 2026-05-18
**Team:** TEAM CONTENT — Event script + triggers
...
- **R45** — Anti-dupe event_id unique
- **R47** — Cross-reference với QUEST/NPC/MAP triggers
- **R49** — Content tagging type + era
- **R50** — Schema-strict event_id 1..600
- **R68** — Deterministic event trigger order
...
```yaml
goal: ">=600 event scripts + 5 era + trigger condition + reward config"

target_event_count: 600
target_festival: 150
target_raid: 100
target_world_boss: 50
target_season: 100
target_limited: 100
target_lore: 100
```
...
```python
#!/usr/bin/env python3
"""CMD_EVENT v1.0 — autonomous builder."""
...
EVENT_TYPES = ['festival', 'raid', 'world_boss', 'season', 'limited', 'lore']
...
def run_full_build():
    ...
    events = []
    eid = 1
    for etype, count in [('festival', TARGET_FESTIVAL), ('raid', TARGET_RAID),
                         ('world_boss', TARGET_WORLD_BOSS), ('season', TARGET_SEASON),
                         ('limited', TARGET_LIMITED), ('lore', TARGET_LORE)]:
        for _ in range(count):
            events.append({'event_id': eid, 'type': etype, 'era': ERAS[eid % len(ERAS)]})
            eid += 1
    ...
    schema_sql = """CREATE TABLE IF NOT EXISTS event_items (
    id INT PRIMARY KEY,
    natural_key VARCHAR(64) NOT NULL,
    UNIQUE(natural_key)
);
CREATE INDEX idx_event_key ON event_items(natural_key);
"""
    (OUTPUT_DIR / 'schema' / f'event_table.sql').write_text(schema_sql, encoding='utf-8')

    # Test stub (≥15 tests required)
    test_code = '''# 15 tests for EVENT
import json
from pathlib import Path
...
'''
    (OUTPUT_DIR / 'tests' / f'event_tests.py').write_text(test_code, encoding='utf-8')
...
**Target:** ≥600
**Extend:** 600 entries mới (existing IMMUTABLE).

```python
def r71_workflow():
    """R71: Tận dụng existing, mở rộng không làm mới."""
    existing_path = REPO_DIR / 'cmd-event' / 'existing' / f'EVENT_0.jsonl'
    ...
    if existing_path.exists():
        ...
        log.info(f'Loaded {len(existing)} existing EVENT from {existing_path}')
    else:
        log.warn(f'Existing registry NOT FOUND at {existing_path} — will generate full 600')
    ...
    # 3. Check target met
    if len(valid_existing) >= 600:
        log.info(f'Target 600 met with existing valid {len(valid_existing)}')
        return valid_existing, 0  # 0 new

    # 4. Extend chỉ phần thiếu
    needed = 600 - len(valid_existing)
    start_id = max(
...
suite = RNGSuite(seed=f'event:{entity_id}')
```
...
1-15: implementation specific to CMD_EVENT (count, schema, cross-ref, idempotency)

### ⚠️ Gap nội tại (4 admit honest)

1. **Event script simple trigger->reward** — MED
2. **Festival không tied calendar** — MED
3. **No GM admin override** — MED
4. **Reward inflation chưa verified** — MED
...
Repo `Trinhlong1988/svtk-status` branch `staging-event-{ts}`.
...
```python
SVTK_TARGET = 600    # VSTK target (vượt TSO)
TSO_BASELINE = 425    # TS Online actual
# Phải PASS: count >= SVTK_TARGET (> TSO 425)
```
...
EXISTING_PATH = REPO_DIR / 'cmd-event' / 'existing'
OUTPUT_PATH = REPO_DIR / 'cmd-event' / 'output' / 'registry'
...
def main_pipeline():
    ...
    if bugs:
        for bug in bugs:
            ...
            send_alert_to_lead_with_target(severity, f'event_' + bug['type'],
                                          bug['evidence'], target_worker='event')
    ...
    out = OUTPUT_PATH / 'event_full.jsonl'
    ...
    push_completion_to_lead(
        fix_id=f'event_extend_to_target',
